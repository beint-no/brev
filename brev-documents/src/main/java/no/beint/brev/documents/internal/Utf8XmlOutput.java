package no.beint.brev.documents.internal;
...
/** Small buffered UTF-8 encoder specialized for compact XML 1.0 output. */
public final class Utf8XmlOutput {
    ...
    public void startOpen(String name) throws IOException {
        raw("<");
        raw(name);
    }
    ...
    public void closeStart() throws IOException {
        raw(">");
    }
    ...
    public void optionalElement(String name, Optional<String> value) throws IOException {
        if (value.isPresent()) {
    ...
    public void base64(byte[] data) throws IOException {
        ...
        while (offset < data.length) {
            ...
            int chunk = Math.min(remaining, 3072);
            if (remaining > chunk) {
                chunk -= chunk % 3;
            }
            byte[] source = offset == 0 && chunk == data.length
                    ? data
                    : java.util.Arrays.copyOfRange(data, offset, offset + chunk);
            byte[] encoded = encoder.encode(source);
            writeBytes(encoded, encoded.length);
            offset += chunk;
        }
    }
    ...
    private void writeBytes(byte[] bytes, int length) throws IOException {
        ...
        while (offset < length) {
            ...
            int copy = Math.min(length - offset, buffer.length - position);
            System.arraycopy(bytes, offset, buffer, position, copy);
            position += copy;
            offset += copy;
        }
    }
